...
|  | `-p <branch>` | 현재 브랜치의 **부모 브랜치를 직접 지정**하여 기록합니다. |
...
또한 `git s -c` 없이 만든 브랜치라도 부모 관계를 직접 등록할 수 있습니다.

* **사용 예시**: `git b -p develop`
* **동작**: 현재 브랜치의 부모를 `develop`으로 `branches.json`에 기록합니다. 이렇게 등록한 관계는 이후 `git r parent`의 컨텍스트로 사용됩니다.
